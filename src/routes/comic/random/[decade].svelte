<script context="module" lang="ts">
	// TODO: lots of duplication between random pages
	import type { RandomResponse } from '$lib/types';

	export async function load({ fetch, page }) {
		const url = `/comic/random/${page.params.decade}.json`;
		...
		if (res.ok) {
			return {
				props: {
					...
					decade: parseInt(page.params.decade)
				}
			};
		}
...
<script lang="ts">
	...
	function showMore() {
		if (allComicsShowing) {
			...
			fetch(`/comic/random/${decade}.json`)
				.then((res) => res.json())
				.then((res) => {
					comics = res.comics;
					index = 0;
...
<h1>Random comic from the {decade}s</h1>
